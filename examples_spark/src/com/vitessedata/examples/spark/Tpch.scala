package com.vitessedata.examples.spark
...
class Tpch(val sc: SparkContext, val sqlc: SQLContext, val scale: String) extends Logging {
  def parquetfn(tbl :String) = "file:///nx/tpch-" + scale + "/parquet/" + tbl

  def prepareParquetTables(): Unit = {
    val nation = sqlc.read.parquet(parquetfn("nation"))
    nation.registerTempTable("nation")

    val region = sqlc.read.parquet(parquetfn("region"))
    region.registerTempTable("region")

    val part = sqlc.read.parquet(parquetfn("part"))
    part.registerTempTable("part")

    val supplier = sqlc.read.parquet(parquetfn("supplier"))
    supplier.registerTempTable("supplier")

    val partsupp = sqlc.read.parquet(parquetfn("partsupp"))
    partsupp.registerTempTable("partsupp")

    val customer = sqlc.read.parquet(parquetfn("customer"))
    customer.registerTempTable("customer")

    val orders = sqlc.read.parquet(parquetfn("orders"))
    orders.registerTempTable("orders")

    val lineitem = sqlc.read.parquet(parquetfn("lineitem"))
...
object Tpch extends Logging {
  def main(args: Array[String]): Unit = {
    ...
    val tpch = new Tpch(sc, sqlc, scale)
...
class TpchLoad(val sc: SparkContext, val sqlc: SQLContext, val scale: String) {
  ...
  def loadNation(): Unit = {
    ...
    val df = sqlc.createDataFrame(tbl, schema)
...
object TpchLoad {
  def main(args: Array[String]): Unit = {
    ...
    val sqlc = new SQLContext(sc)
    val tpch = new TpchLoad(sc, sqlc, scale)
    tpch.loadTextTables()
  }
}
